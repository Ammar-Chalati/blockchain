...
export default function Register() {
	...
	async function submit(e){
		...
		if(password === passwordConfirmation){
			try {
				await axios.post("http://localhost:9982/auth/sign-up", {
					firstname: fname,
					lastname: lname,
					phone_number: number,
					email: email,
				...
				<div className='container'>
					<div className='parent'>
						<form onSubmit={submit} className='card'>
							...
							{fname.length < 2 && accept && <p className='error'>Name must be more than 2 char</p>}
							<label htmlFor="lname">Last Name: </label>
							<input id="lname" type='text' placeholder='Last Name...' value={lname} onChange={(e) => setLname(e.target.value)} required/>
							{lname.length < 2 && accept && <p className='error'>Name must be more than 2 char</p>}
							...
							<input id="passwordCo" type='password' placeholder='Password Confirmation...' value={passwordConfirmation} onChange={(e) => setPasswordConfirmation(e.target.value)} />
