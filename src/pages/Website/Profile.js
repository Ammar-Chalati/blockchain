  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await Promise.all([
          axios
            .get(`http://localhost:9982/Users/current`,{
  ...
  const showPrices = pricesnew.map((price ,index) => (
    <div className='item' key={index}>
      <div className='left'>
        ...
        <div className='title'>
          <h6>{price.name}</h6>
          <p>$ {(price.current_price).toLocaleString()} {price.price_change_percentage_24h >= 0 &&<span style={{marginLeft:'15px',color:"green"}}>+{(price.price_change_percentage_24h).toLocaleString()} %</span>}{price.price_change_percentage_24h < 0 &&<span style={{marginLeft:'15px',color:"red"}}>{(price.price_change_percentage_24h).toLocaleString()} %</span>}</p>
        </div>
      </div>
      <div className='right'>
        <h6>{(price.name === 'Bitcoin'? balance : 0 )} {(price.symbol).toUpperCase()}</h6>
        <p>$ {(price.name === 'Bitcoin'? balance * price.current_price : 0).toLocaleString()}</p>
      </div>
    </div>
  ))
  ...
  return (
    <>
          ...
          <div className='father'>
            ...
            <div className='icon'>
              <div onClick={clickdeposit}>
                <i className="fa-solid fa-down-long shadow" style={{rotate:'180deg'}}></i>
                <p>Deposit</p>
              </div>
      ...
      <div className='windowWith card'>
        ...
        <div className='part'>
          <p>The withdrawal process is not available at the moment ,please contact the customer support.</p>
        </div>
      </div>
    </>
  )
